...
def gzip_to_dataframe(gz_file):
    ...
    df[['year', 'month', 'day']] = pd.to_datetime(df['YEARMODA'], format='%Y%m%d').astype('str').str.split('-', expand=True).astype('int64')

    # extract weather indicators and save into corresponding weather event columns
    df[['fog', 'rain', 'snow', 'hail', 'thunder', 'tornado']] = df['FRSHTT'].astype(str).apply(list).apply(pd.Series).fillna(0).astype(int)

    # extract precipitation values and flags and store in respective columns
    df[['precip_in', 'precip_flag']] = df['PRCP'].str.extract('(?P<precip_in>[\d.]{4})(?P<precip_flag>[A-Z]{1})').fillna({'precip_in':0, 'flag':np.nan})
    ...
    df = df.drop(columns=['FRSHTT', 'YEARMODA', 'MAX', 'MIN', 'PRCP'])
